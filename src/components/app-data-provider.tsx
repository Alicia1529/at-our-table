...
export function AppDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const addPhotos = useCallback(async (dinnerId: string, files: File[]) => {
    ...
    for (const file of files) { const id = uid(); let url = ""; if (live) { const supabase = createClient(); const { data: userData } = await supabase.auth.getUser(); if (!userData.user) throw new Error("Sign in to upload photos."); const path = `${state.space.id}/${dinnerId}/${id}-${safeName(file.name)}`; const { error: uploadError } = await supabase.storage.from("dinner-media").upload(path, file, { contentType: file.type }); if (uploadError) throw uploadError; const { error } = await supabase.from("photos").insert({ id, space_id: state.space.id, dinner_id: dinnerId, uploaded_by: userData.user.id, storage_path: path, caption: file.name }); if (error) throw error; const { data } = await supabase.storage.from("dinner-media").createSignedUrl(path, 3600); url = data?.signedUrl ?? await fileToDataUrl(file); } else url = await fileToDataUrl(file); photos.push({ id, dinnerId, url, caption: file.name, createdAt: new Date().toISOString() }); }
