...
export function GoogleSignIn({ nextPath = "/" }: { nextPath?: string }) {
  ...
  return <div><button type="button" onClick={signIn} className="focus-ring flex w-full items-center justify-center gap-3 rounded-full bg-white px-5 py-3.5 text-sm font-bold text-[var(--ink)] shadow-sm"><span className="grid size-6 place-items-center rounded-full border hairline font-serif text-[var(--tomato)]">G</span>Continue with Google</button>{message && <p role="status" className="mt-3 text-center text-xs leading-5 text-white/65">{message}</p>}</div>;
}
